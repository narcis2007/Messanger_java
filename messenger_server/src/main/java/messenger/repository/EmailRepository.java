...
import messenger.repository.data.AbstractPagingAndSortingRepository;
...
public class EmailRepository extends AbstractPagingAndSortingRepository<Email,Integer>{
	...
	protected Email saveXmlElement(Element eElement) {
		...
		Email email=new Email(title,content,Integer.valueOf(owner));
	...
	public Node child(Email e) {
		...
		owner.appendChild(doc.createTextNode(String.valueOf(e.getSender())));
